...
from encoder_my import *
from utils import *
import sys


class TMAGCN(nn.Module):
    def __init__(self,
                 node_embeddings,
                 # n_event_types,
                 N_nodes,
        ...
        n_in = 0
        self.z_struct = nn.Linear(n_hidden * self.n_assoc_types, n_hidden)
        self.z_linear = nn.Linear(2 * n_hidden, n_hidden)
        self.z_bilinear = nn.Bilinear(n_hidden, n_hidden, n_hidden)
        ...
        self.GCN = GcnNet(input_dim=self.n_hidden, hid_dim=self.n_hidden, output_dim=n_hidden).to(device)
        self.TMA_GCN = TMA_GcnNet(input_dim=self.n_hidden, hid_dim=self.n_hidden, output_dim=n_hidden).to(device)
        # self.TMAGAT = GAT(nfeat=self.n_hidden, nhid=self.n_hidden, dropout=0.6, alpha=0.2, nheads=8)
        # self.TMAGAT = GAT(nfeat=n_hid, nhid=n_hid, nclass=3, dropout=0.6, alpha=0.2, nheads=8)
        if encoder is not None:
            # self.TMAGCN = GcnNet(input_dim=self.n_hidden, hid_dim=self.n_hidden, output_dim=32).to(device)
    ...
    def initialize(self, node_embeddings, A_initial, keepS=False):
        ...
        # if A_initial is None:
            ...
            self.adjacency = torch.from_numpy(A_initial).float().to(self.device)
    ...
    def intensity_rate_lambda(self, z_u, z_v, k):
        ...
        g_psi = torch.clamp(g / (psi + 1e-7), -75, 75)  # to prevent overflow将输入input张量每个元素的夹紧到区间 [min,max][min,max]，并返回结果到一个新张量。
    ...
    def get_degree(self, node1, node2):
        ...
        for c, (v, u) in enumerate(zip([node1, node2], [node2, node1])):  # i is the other node involved in the event
    ...
    def update_node_embed(self, prev_embed, node1, node2, time_delta_uv, k):
        ...
        z_mid = prev_embed.new(self.N_nodes, self.n_hidden, self.n_assoc_types).fill_(0)
        z_mid_GCN = prev_embed.new(self.N_nodes, self.n_hidden, self.n_assoc_types).fill_(0)
        z_mid_TMA = prev_embed.new(self.N_nodes, self.n_hidden, self.n_assoc_types).fill_(0)
        h_u_struct = prev_embed.new(2, self.n_hidden, self.n_assoc_types).fill_(0)
        h_u_gcn = prev_embed.new(2, self.n_hidden, self.n_assoc_types).fill_(0)
        for c, (v, u, delta_t) in enumerate(zip([node1, node2], [node2, node1], time_delta_uv)):  # i is the other node involved in the event
            node_degree[u] = np.zeros(self.n_assoc_types)
            for rel in range(self.n_assoc_types):
                if self.latent_graph:
                    Neighb_u = self.S[u, :, rel] > 1e-7
                    adjacency = self.S[:, :, rel].squeeze()
                    # print(Neighb_u)
                else:
                    Neighb_u = self.A[u, :, rel] > 0  # when update embedding for node v, we need neighbors of u and vice versa!
                    adjacency = self.A[:, :, rel].squeeze()
                N_neighb = torch.sum(Neighb_u).item()  # number of neighbors for node u
                node_degree[u][rel] = N_neighb
                z_mid_GCN[:, :, rel] = self.GCN(adjacency, node_embed).detach()
                z_mid_TMA[:, :, rel] = self.TMA_GCN(adjacency, node_embed).detach()
                ...
                # # 1 
                z_mid[:, :, rel] = z_mid_GCN[:, :, rel] + z_mid_TMA[:, :, rel]
                # # 2   
                # z_mid[:, :, rel] = self.z_linear(torch.cat((z_mid_GCN[:, :, rel], z_mid_TMA[:, :, rel]), dim=1)).detach()
                # # 3 
                # z_mid[:, :, rel] = self.z_bilinear(z_mid_GCN[:, :, rel].squeeze(), z_mid_TMA[:, :, rel].squeeze()).detach()
                
                h_u_gcn[:, :, rel] = z_mid[[node1, node2], :, rel]
        ...
        h4 = self.W_struct(h_u_gcn.view(2, self.n_hidden * self.n_assoc_types))

        z_new[[node1, node2], :] = torch.sigmoid(h1 + h2 + h3 + h4)
        ...
        for c, j in enumerate([u, v]):  # range(i + 1, N):
            for i in range(N):
                ...
                if t_bar < time_keys_min:
                    ...
                    # t_bar is somewhere in between time_keys_min and time_keys_max
    ...
    # def generate_S(self, prev_embed, u, v, adjacency, tor_M, MC, train_enc=False):
    def generate_S(self, prev_embed, u, v, train_enc=False):
        ...
        if not train_enc:
            ...
            with torch.no_grad():
                ...
                # logits, idx = self.encoder(prev_embed, adjacency, tor_M, MC, edges=edges)
    ...
    def forward(self, data):
        ...
        for it, k in enumerate(event_types):
            ...
            node_degree, z_new = self.update_node_embed(z_prev, u_it, v_it, time_delta_uv[it], k)
        ...
        return lambda_uv_t, lambda_uv_t_non_events / self.N_surv_samples, [A_pred, Surv], S_batch, self.z, reg