...
typedef TTriple<TInt, TInt, TIntPair> TThree;
typedef TTriple<TIntQu, TIntPair, TInt> TRecord;//TRecord <TIntQu, TIntPair, TInt>:<(lable,i,j,event), uv, timestamps>
typedef TTriple<TIntQu, TIntPair, TIntPair> TRecordStar;//TRecordStar <TIntQu, TIntPair, TInt>:<(level,i,j,event), uv, (lable,timestamps)>
...
class TempMotifCounter {
 public:
  ...
  void Count3TEdge2Node(double delta, Counter2D& counts, TVec<TVec<TStr>>* motifRecordFile);
  ...
  void Count3TEdge2Node(int u, int v, double delta, Counter3D& counts, TVec<TVec<TStr>>* motifRecordFile);
  ...
  void Count3TEdge3NodeStars(double delta, Counter3D& pre_counts,
                             Counter3D& pos_counts, Counter3D& mid_counts, TVec<TVec<TStr>>* motifRecordFile);
  ...
  void Count3TEdgeTriads(double delta, Counter3D& counts, TVec<TVec<TStr>>* motifRecordFile);
  ...
  void Count3TEdgeTriadsNaive(double delta, Counter3D& counts, TVec<TVec<TStr>>* motifRecordFile);
  ...
  void Count3TEdge23Node(double delta, Counter2D& counts, TVec<TVec<TStr>>* motifRecordFile);
 ...
 private:
  ...
  void AddStarEdgeData(TVec<TIntPair>& ts_indices, TVec<TThree>& events,
		       int& index, int u, int v, int nbr, int key);
  // Another simple wrapper for adding star edge data
  //void AddStarEdges(TVec<TIntPair>& combined, int u, int v, int key);
  void AddStarEdges(TVec<TThree>& combined, int u, int v, int key);
...
class ThreeTEdgeMotifCounter {
 public:
  ...
  void Count(const TIntV& event_string, const TIntV& timestamps, const TVec<TIntPair>& uv,
 ...
 private:
  ...
  void IncrementCounts(int event, TIntPair uv, TVec<TRecord> *MotifR, TInt timestamps, TVec<TVec<TStr>>* motifRecordFile);
  void DecrementCounts(int event, TIntPair uv, TVec<TRecord> *MotifR, TInt timestamps);
  void PrintMotif(TVec<TRecord> *MotifR, TVec<TVec<TStr>>* motifRecordFile);
  Counter1D counts1_;
  Counter2D counts2_;
  Counter3D counts3_;
...
class StarTriad3TEdgeCounter {
 public:
  ...
  void Count(const TVec<TThree>& events, const TIntV& timestamps, double delta, TVec<TVec<TStr>>* motifRecordFile);
  
 protected:
  // These methods depend on the motif type (star or triad).
  virtual void InitializeCounters() = 0;
  virtual void PopPre(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR) = 0;
  virtual void PopPos(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR) = 0;
  virtual void PushPre(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR) = 0;
  virtual void PushPos(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR) = 0;
  virtual void ProcessCurrent(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR, TVec<TVec<TStr>>* motifRecordFile) = 0;
};

// Class for counting star motifs with a given center node.
class ThreeTEdgeStarCounter : public StarTriad3TEdgeCounter<TThree> {
 public:
 ...
 protected:
  ...
  void PopPre(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR);
  void PopPos(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR);
  void PushPre(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR);
  void PushPos(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR);
  void ProcessCurrent(const TThree& event, const TInt& timestamps, TVec<TRecordStar>* MotifR, TVec<TVec<TStr>>* motifRecordFile);
  void PrintStarMotif(TVec<TRecordStar>* MotifR, TVec<TVec<TStr>>* motifRecordFile);
