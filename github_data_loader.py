...
class GithubDataset(EventsDataset):
    ...
    def __init__(self, split, data_dir='./Github'):
        ...
        filepath = data_dir+"1d/"
        if split == 'train':
            time_start = 0
            # time_end = datetime(2013, 8, 31, tzinfo=self.TZ).toordinal()
            time_end = datetime(2013, 5, 31, tzinfo=self.TZ).toordinal()
        elif split == 'test':
            # time_start = datetime(2013, 9, 1, tzinfo=self.TZ).toordinal()
            # time_end = datetime(2014, 1, 1, tzinfo=self.TZ).toordinal()
            # time_start = datetime(2013, 6, 1, tzinfo=self.TZ).toordinal()
            # time_end = datetime(2013, 8, 1, tzinfo=self.TZ).toordinal()
            time_start = datetime(2013, 6, 1, tzinfo=self.TZ).toordinal()
            time_end = datetime(2013, 7, 1, tzinfo=self.TZ).toordinal()
        else:
        ...
        # self.TEST_TIMESLOTS = [datetime(2013, 6, 10, tzinfo=self.TZ),
        #                        datetime(2013, 6, 20, tzinfo=self.TZ),
        #                        datetime(2013, 6, 30, tzinfo=self.TZ),
        #                        datetime(2013, 7, 10, tzinfo=self.TZ),
        #                        datetime(2013, 7, 20, tzinfo=self.TZ),
        #                        datetime(2013, 7, 31, tzinfo=self.TZ)]
        self.TEST_TIMESLOTS = [datetime(2013, 6, 5, tzinfo=self.TZ),
                               datetime(2013, 6, 10, tzinfo=self.TZ),
                               datetime(2013, 6, 15, tzinfo=self.TZ),
                               datetime(2013, 6, 20, tzinfo=self.TZ),
                               datetime(2013, 6, 25, tzinfo=self.TZ),
                               datetime(2013, 6, 30, tzinfo=self.TZ)]
        ...
        initial_embeddings = np.zeros((284,36))
        for m in range(36):
