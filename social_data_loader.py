...
class SocialEvolutionDataset(EventsDataset):
    ...
    FIRST_DATE = datetime.datetime(2008, 10, 1)
    ...
    def __init__(self,
        ...
        self.TEST_TIMESLOTS = [datetime.datetime(2008, 12, 5), datetime.datetime(2008, 12, 10),datetime.datetime(2008, 12, 15),
                               datetime.datetime(2008, 12, 20), datetime.datetime(2008, 12, 25),datetime.datetime(2008, 12, 31)]
    ...
    def load_data(data_dir, prob, dump=True):
        data_file = pjoin(data_dir, 'data_prob%s_20081001.pkl' % prob)
...
class CSVReader:
    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            time_end = datetime.datetime(2008, 11, 30).toordinal()
        elif split == 'test':
            if test_slot != 1:
            ...
            time_start = datetime.datetime(2008, 12, 1).toordinal()
            time_end = datetime.datetime(2008, 12, 31).toordinal()
        else:
            time_start = 0
            time_end = np.Inf

        csv = pandas.read_csv(csv_path,encoding='unicode_escape')
        self.data = {}
